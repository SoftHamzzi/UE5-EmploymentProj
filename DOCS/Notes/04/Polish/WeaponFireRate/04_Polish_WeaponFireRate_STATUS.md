...
## 1. 현재 확정 설계 (2026-09-23 기준 — **안 B + UT식 원점 동기 + 탄약 예측**, 역할 분기 없는 단일 경로. 안 C 폐기)
...
| 09-23 | **역할 분기 제거** (사용자 결정). `FireOnce`의 `IsNetAuthority()` 분기 + `ServerConfirmOneShot` + `SendFireTargetData` → `OnTargetDataReady` 하나로. 계기: 그 분기에 `return`이 빠져 호스트가 `CommitAbilityCost`를 두 번 부르던 버그. Lyra는 같은 문제를 구조로 없앤다 — 로컬 타겟팅도 같은 콜백으로 들어가고(`:596`) 전송 여부만 `IsLocallyControlled() && !IsNetAuthority()`로 가른다(`:489`). 우리와 다른 점 셋: (a) Lyra는 활성화당 한 발이라 `CommitAbility`를 그 안에 그냥 두지만 우리는 발마다라 역할 조건 밖에 둬야 하고, (b) 버킷이 차감보다 앞서야 하며, (c) Lyra는 발마다 새 예측 키를 만들지만(`:484` 기본 `CanGenerateNewKey=true`) **배칭을 켜지 않는다**(`grep ShouldDoServerAbilityRPCBatch` → TODO 주석뿐, `LyraAbilitySystemComponent.cpp:227`) — 우리는 Step 14에서 켜므로 첫 발 활성화 키 재사용을 유지한다 | `LyraGameplayAbility_RangedWeapon.cpp:484, :489, :531, :596`, `LyraAbilitySystemComponent.cpp:227`, `GameplayPrediction.cpp:406` |
| 09-23 | **정정** — 09-22의 "종속 키를 만들면 첫 발 예측 GE가 남는다"는 **기본 설정에서는 틀렸다**. `FPredictionKeyDelegates::AddDependency`가 베이스 ack을 종속 키로 전파하고(`GameplayPrediction.cpp:355-357`), CVar `AbilitySystem.PredictionKey.DepChainBehavior` 기본값 1에서 그 경로가 켜져 있다. 엔진 주석이 그 줄의 존재 이유로 `FScopedServerAbilityRPCBatcher`를 직접 지목한다(`:351-354`). 다만 같은 주석이 "논리적으로 옳지 않다"며 목표값을 3으로 적었고, 3에서는 전파가 끊긴다. **결론(첫 발 활성화 키 재사용)은 그대로** — 이유가 "안 지워진다"에서 "레거시 동작에 의존하게 된다"로 바뀐다. 개념 정리는 `DOCS/Mine/Concepts/PredictionKey.md` | `GameplayPrediction.cpp:31-33, :338-358` |
