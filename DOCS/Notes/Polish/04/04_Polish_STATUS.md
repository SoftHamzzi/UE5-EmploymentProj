...
- [ ] **안 B(`UAbilityTask_NetworkSyncPoint`)** — 클라에서 힐/실드/쿨다운
  ...
  설계 확정, 코드 미적용. `04_Polish_SkillDisplay.md` §2
- [ ] (신호 있으면) 재발동 자체의 핑 공정성 — GAS가 쿨다운을 진짜로
  예측 못 하는 근본 한계. 방향만 남겨둠. `04_Polish_SkillDisplay.md` §3
- [ ] `Entry.State.Charges` 이관 — Step 05(`05_Loot_05_Equipment.md`) 차례,
  Step 03/04 완성 후. `04_Polish_WeaponFireRate.md` §3
- [ ] `FireMode::Burst` 미구현. `04_Polish_WeaponFireRate.md` §3
...
```
@GAS_STATUS.md @Polish/04/04_Polish_STATUS.md @Polish/04/04_Polish_XXX.md
이어서 진행.
```
