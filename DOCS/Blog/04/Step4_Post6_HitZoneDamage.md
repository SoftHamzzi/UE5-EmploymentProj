...
**이 포스팅에서 다루는 것:**
- `bIsWeakSpot` bool + 본 이름 문자열 매칭의 한계
...
| 본 이름은 스켈레톤 종속 | `"thigh_l"`은 UE5 마네킹 규약이다. 다른 스켈레톤 캐릭터를 넣으면 매칭이 전부 깨진다 |
| 두 시스템이 겹친다 | 본 이름 배율과 약점 플래그가 **같은 히트에 둘 다 곱해진다.** 의도치 않은 중복 |
| 배율의 소유자가 애매하다 | `BoneDamageMultiplierMap`은 무기에 있는데, 키가 캐릭터의 본 이름이다 |

마지막 항목이 핵심이다. **무기 데이터가 캐릭터의 뼈 이름을 알아야 하는 구조**였다.
...
> 참고로 이전 `BoneDamageMultiplierMap`은 **UPROPERTY도 아니었다** — 에디터에서 편집조차 안 되는 C++ 하드코딩이었다. 태그 방식으로 오면서 비로소 디자이너가 만질 수 있게 됐다.
...
- Step 3-1 / 3-3 포스팅 — Physics Asset 구성, 원래의 부위 배율 구현
