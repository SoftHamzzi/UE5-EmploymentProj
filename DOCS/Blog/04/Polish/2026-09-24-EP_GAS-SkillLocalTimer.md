...
쿨다운 게이지가 5에서 4.5까지 내려가다 5로 되돌아가는 문제는 표시를 GE에서 떼어내
해결했다. 어빌리티가 쿨다운을 적용하는 자리에서 지속시간을 메시지로 발행하고, 위젯은
받은 순간의 로컬 시계를 시작점으로 잡는다.

그런데 실제 게이트는 그대로였다. 게이지가 0에 닿아도 다시 누르면 아무 일도 안 일어나는
구간이 있었고, 핑이 높을수록 그 구간이 길었다.

<!-- [스크린샷 1] 게이지는 0인데 눌리지 않는 구간
     찍는 법: PktLag 200 PktLagVariance 0으로 PIE 2인, 클라에서 스킬 사용 후
     게이지가 0에 닿는 순간부터 연타. 눌려도 아무 일이 없는 구간이 보여야 한다.
     이 빌드는 이미 로컬 타이머로 바뀌었으므로 ea08cfc 이전 커밋에서 찍거나,
     못 찍으면 이 자리는 비워두고 문장만 남긴다. -->
![cooldown_gate_blocked.gif](GITHUB_ASSET_URL)
...
시계는 `World->GetTimeSeconds()`를 쓴다. 값이 기계를 건너가지 않으니 서버 시각을 추정할
...
타임스탬프가 RPC로 기계를 건너갈 때다.

## 타임스탬프가 아니라 남은 시간을 든다

처음엔 `LastActivationTime` 하나면 될 것으로 봤다. 쿨감이 들어오면서 바뀌었다.

이 프로젝트의 쿨타임 감소는 세 갈래다. 어트리뷰트 두 개(고정값 감소, 퍼센트 감소)는
...
틱이 없다. 조회 시점에 계산한다. `Rate`를 타이머가 저장하지 않고 조회자가 넘기는 것은,
...
이 저장소는 쿨감뿐 아니라 캐스팅 중 이동속도, 무기 연사 속도까지 같이 쓴다. 한 값을 두
...
때문이다. 권위 쪽은 자기가 판단하는 주체다.

## 캐스팅 잠금은 태그와 배율로 나눠 담았다
...
| 이동속도 50% | `LocalModifiers.Set(Modifier.MoveSpeed.Casting)` |
...
배율과 로컬 배율을 둘 다 곱하도록 해서 CMC가 바로 읽는다.

<!-- [스크린샷 2] 캐스팅 중 이동속도 감소와 다른 스킬 잠김
     찍는 법: 힐(3초) 시전 중 이동하면서 Dash/Shield 키를 눌러본다.
     이동이 느려지고 두 스킬이 안 나가는 것, 시전이 끝나면 바로 풀리는 것까지 한 컷. -->
![casting_lock.gif](GITHUB_ASSET_URL)

3초 채널링이 끝나는 시점을 클라이언트와 서버가 맞추는 일은 `UAbilityTask_NetworkSyncPoint`에
맡겼다. `OnlyServerWait`로 걸면 클라이언트는 기다리지 않고 바로 다음으로 가고, 서버는
클라이언트 신호를 기다린다. 이 태스크가 예측 창까지 열어주므로 힐 GE가 클라이언트에서
예측 적용된다.

## 결과

세 스킬(Dash, Heal, ShieldOn)이 같은 베이스 위에서 돌고, 서브클래스는 `OnCastComplete()`
하나만 오버라이드한다. 쿨다운 GE와 캐스팅 GE 에셋은 지웠다.

재발동 차단이 클라이언트 로컬 판정이 되면서, 게이지가 0에 닿은 순간 실제로 다시 누를 수
있게 됐다. 서버가 0.1초 여유를 두고 같은 판정을 하므로 정상 플레이에서 거절이 나지 않는다.

<!-- [스크린샷 3] 같은 조건에서 게이지 0과 동시에 재발동
     찍는 법: 스크린샷 1과 같은 PktLag 200 조건, 같은 스킬. 게이지가 0에 닿는 프레임에
     바로 다시 나가는 것이 보여야 한다. 스크린샷 1과 나란히 두는 것이 목적이므로
     같은 화면 구도, 같은 스킬 슬롯으로 찍는다. -->
![cooldown_gate_open.gif](GITHUB_ASSET_URL)

**아직 PIE 실측을 돌리지 않았다.** 쿨다운 역행이 사라졌는지, 거절 롤백이 실제로 도는지,
캐스팅 중 이동속도가 양쪽에서 같은지는 체크리스트로 남겨뒀다. 여기까지는 코드가 설계대로
들어갔다는 것만 확인한 상태다.
...
1. `EndAbility` 오버라이드가 이동속도 배율을 지우는 코드를 `Super` 호출보다 앞에 두고
   ...
   사이 시작된 새 캐스팅의 배율을 지울 수 있다. 함수 첫 줄에 가드를 넣으면 닫힌다.
...
착각하지 않는 게 중요했다"고 적었는데, 그 문장을 실제로 갚는 데 시간이 걸렸다.
